...
$P(H_i)$ is called the *prior probability*, $P(H_i\mid A)$ is the *posterior probability*.

### Derivation
- Suppose a trial has been conducted, as a result of which event $A$ has occurred.
- Let us find the conditional probabilities $P(H_1\mid A), P(H_2\mid A), ..., P(H_n\mid A)$
  - That is, the probabilities of the hypotheses, given that event $A$ has already occurred.
- For $P(H_1\mid A)$, by the multiplication theorem we have
  - $P(A H_1) = P(A) P(H_1 \mid A) = P(H_1) P(A \mid H_1)$
  - That is, $P(H_1 \mid A) = \frac{P(H_1) P(A \mid H_1)}{P(A)}$
- If we replace all $P(A)$ with the total probability formula, we get
  - $P(H_1 \mid A) = \frac{P(H_1) P(A \mid H_1)}{P(H_1) P(A\mid H_1) + P(H_2) P(A\mid H_2) + ... + P(H_n) P(A\mid H_n)}$
- Similarly for the other hypotheses. That is,
  - $P(H_i \mid A) = \frac{P(H_i) P(A \mid H_i)}{P(H_1) P(A\mid H_1) + P(H_2) P(A\mid H_2) + ... + P(H_n) P(A\mid H_n)}$
...
Let $H_1$ be the hypothesis that the coin was drawn from the first box, $H_2$ from the second, and $H_3$ from the third. Obviously, $P(H_1) = P(H_2) = P(H_3) = \frac{1}{3}$.

$D$ - the drawn coin is gold. Then $P(D\mid H_1) = 1$, $P(D\mid H_2) = 0.5$ and $P(D\mid H_3) = 0$.

We compute $P(H_1\mid D)$ using the formula $P(H_1\mid D) = \frac{ P(H_1) P(D\mid H_1) }{ \sum_{i=1}^{3} P(H_i) P(D\mid H_i) }$
